...
$target_path = "training/";
     ...
     if (isset($_POST['id'])){
    $id=$_POST["id"];
     $user = $db->UpdateTraining($id,$Details,$Links,$User,$IP,$Photo);
        if ($user) {  
          $_SESSION["error"]=1;
     ...
     $user = $db->addTraining($Title,$Details,$Links,$User,$IP,$Photo);
             if ($user) {  
          $_SESSION["error"]=1;
        $response["error"] = false;
        header('Location: http://139.59.38.160/Ecosense/Dashboard/StaffTraining.php');
    } else  {
            $_SESSION["error"]=2;
        $response["error"] = true;
          header('Location: http://139.59.38.160/Ecosense/Dashboard/StaffTraining.php');
