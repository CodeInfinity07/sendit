...
===================
  ...
  * Fixed #219 - Fix valid() on elements with dependency-callback or dependency-ion.
...
* Added 'pattern' to additional-methods to validate against arbitrary regular ions.
...
* Added ignore-option, eg. ignore: "[@type=hidden]", using that ion to exclude elements to validate. Default: none, though submit and reset buttons are always ignored
...
* Improved dependency feature by adding two custom ions: ":blank" to select elements with an empty value and �:filled� to select elements with a value, both excluding whitespace
...
* Added dependency feature: The requires method accepts both String (jQuery ions) and Functions as the argument
