...
class Processor:
  ...
  def merge(self, config, data_source):
    ...
    for rule in config['rules']:
      ion = data_source.parse_manager.parse( rule['where'] )
      geometries = filter(lambda g: ion(g.properties), data_source.geometries)
  ...
  def remove(self, config, data_source):
    ion = data_source.parse_manager.parse( config['where'] )
    data_source.geometries = filter(lambda g: not ion(g.properties), data_source.geometries)
