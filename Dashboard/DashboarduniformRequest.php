                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="card" >
                         <div class="card-header">
                                <strong class="card-title">Request Uniform</strong>
                            </div>
                            <div class="card-body" >
                      ...
                      $conn = $db->connect();
                       ...
                       $users =$conn->query("SELECT `ID`,c.AdminAccepted, (SELECT ID FROM admin_login_data WHERE ID=c.StaffID) AS UserID,(SELECT FirstName FROM admin_login_data WHERE ID=c.StaffID) AS StaffID,(SELECT LastName FROM admin_login_data WHERE ID=c.StaffID) AS LastName,(SELECT Color FROM uniformcolor WHERE ID=c.ColorID) AS ColorID, (SELECT Size FROM uniformsize WHERE ID=c.SizeID) AS Size, `message`, `Date`, `Time`, `User`, `IP`, `Accepted` FROM `request_uniform` c ");
                    ...
                    <tr>
                        ...
                        <td><?php echo $user['StaffID'].$user["LastName"]; ?></td>
                            <td><?php echo $user['ColorID']; ?></td>
                          <td><?php echo $user['Size']; ?></td>
                        <td><?php echo $user['message']; ?></td>
                           <td style="color: red;font-weight: bold;"><?php if ($user['Accepted']==0){
                            echo "Pending";
...
function changedata(UserID,id,value) {
 ...
 var id='id='+ id+'&value='+value+'&where=1';
...
request.onreadystatechange = function(response) {
  if (request.readyState === 4) {
    if (request.status === 200) {
          ...
          window.location.replace('http://139.59.38.160/Ecosense/Dashboard/notifyRequest.php?id='+UserID+'&msg=Please check update on your uniform reuest');
...
request.open('POST', 'acceptuniform.php', true);
