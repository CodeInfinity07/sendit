        ...
        <div class="breadcrumbs">
            ...
            <div class="col-sm-8">
                <div class="page-header float-right">
                    <div class="page-title">
                         <button  class="btn btn-primary btn-sm" onclick="location.href = 'http://139.59.38.160/sendit/Dashboard/Deactivated.php';" style="height: 80px;">
                                                            <i class="fa fa-dot-circle-o"></i> Deactivated products
                                                        </button>         

                          <button  class="btn btn-danger btn-sm" onclick="location.href = 'http://139.59.38.160/sendit/Dashboard/AddFinalService.php';" style="height: 80px;">
                                        ...
                                        <tr>
                                            ...
                                            <th>Discount</th>
                                              ...
                                              <th>Image</th>
                                                      ...
                                                      <th style="display: none;">Out of stock</th>
 <th></th>
  <th></th>
 <th></th>
                      ...
                      $conn = $db->connect();
                       ...
                       $users =$conn->query("SELECT `ID`,(SELECT `Name` FROM `tez_Canteen` WHERE ID=f.IDCanteen)AS `Vendor`,(SELECT `Name` FROM `subsubmenu` WHERE ID=f.IDSubmenu)AS `IDsubMenu`,(SELECT `Name` FROM `menu_type` WHERE ID=f.IDMenu)AS `IDMenu`,(SELECT `Category` FROM `submenu` WHERE  ID=f.IDSubsubmenu) AS  `IDSubsubmenu`, `Name`, `Weight`,`Unit`, `Description`,`MRP`, `JalpanPrice`, `Discount`,isOutOfStock, `Photo`,Recomended FROM `foods` f WHERE `Available`=1");
                    ...
                    <tr>
                        ...
                        <td><?php echo $user['Discount']; ?></td>
                          ...
                          <td><img src="<?php echo $user["Photo"];?>" width='48' height='48'></td>
            
          <td style="display: none;"><?php if($user['isOutOfStock']==1){?>
...
function handleClick(cb,id) {
  ...
  window.location.replace("http://139.59.38.160/sendit/Dashboard/out_of_stock.php?id="+id+"&cb="+1);
  } 
  }else{
     if (confirm("Please confirm if the selected item is out of stock")) {
  window.location.replace("http://139.59.38.160/sendit/Dashboard/out_of_stock.php?id="+id+"&cb="+0);
  } 
  }
